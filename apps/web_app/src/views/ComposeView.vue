...
function formatForwardBody(
  ...
  let forwardText = '\n\n';
  forwardText += '-------- 転送メッセージ --------\n';
  forwardText += `From: ${originalMessage.from}\n`;
  forwardText += `To: ${formatEmailList(originalMessage.to)}\n`;
  if (originalMessage.cc && originalMessage.cc.length > 0) {
    forwardText += `Cc: ${formatEmailList(originalMessage.cc)}\n`;
  }
  forwardText += `Date: ${dateStr}\n`;
  forwardText += `Subject: ${originalMessage.subject}\n`;
  forwardText += '\n';

  if (decryptedBody.bodyText) {
    forwardText += decryptedBody.bodyText;
  } else if (decryptedBody.bodyHtml) {
    // HTMLの場合は、簡単にテキストに変換（完全ではない）
    forwardText += decryptedBody.bodyHtml.replace(/<[^>]*>/g, '');
  }
